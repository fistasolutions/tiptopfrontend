import React from 'react';

interface CustomerSatisfactionProps {
  progress?: number;
  modules?: { name: string; completed: boolean }[];
}

const CustomerSatisfaction: React.FC<CustomerSatisfactionProps> = ({
  progress = 64,
  modules = [
    { name: 'Module 1: NPS Training', completed: true },
    { name: 'Module 2: Feedback Loops', completed: false },
    { name: 'Module 3: Customer Journey', completed: false },
  ],
}) => {
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 w-full max-w-xl">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-lg font-semibold text-[#2A3F54]">Customer Satisfaction</h3>
        <span className="text-sm text-gray-500 font-medium">{progress}%</span>
      </div>
      <div className="w-full bg-gray-200 rounded-full h-3 mb-4">
        <div
          className="bg-[#10B981] h-3 rounded-full transition-all duration-500"
          style={{ width: `${progress}%` }}
        ></div>
      </div>
      {modules && modules.length > 0 && (
        <ul className="space-y-2 mt-2">
          {modules.map((mod, idx) => (
            <li key={idx} className="flex items-center text-sm text-gray-700">
              <span className={`mr-2 w-3 h-3 rounded-full border ${mod.completed ? 'bg-[#10B981] border-[#10B981]' : 'bg-white border-gray-300'}`}></span>
              {mod.name}
            </li>
          ))}
        </ul>
      )}
      
    </div>
  );
};

export default CustomerSatisfaction; 